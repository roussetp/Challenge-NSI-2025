...
class GeoGame {
    ...
    initializeElements() {
        ...
        this.backgroundImage.style.height = '105%';
    ...
    handleMapZoom(e) {
        ...
        const xPercent = (x / rect.width) * 80;
        const yPercent = (y / rect.height) * 101;
        // Appliquer le zoom et le déplacement
        this.backgroundImage.style.transformOrigin = `${xPercent}% ${yPercent}%`;
        this.backgroundImage.style.transform = `scale(${GAME_CONFIG.ZOOM_INTENSITY})`;
    }
    ...
    showLeavePopup() {
        const userChoice = confirm("Vous avez atteint le Deuxième niveau! Voulez-vous rester ou quitter?     (clickez sur OK pour rester et Annuler pour quitter");
        if (!userChoice) {
